...
A record `PlainRec xs` is a subtype of a record `PlainRec ys` if `ys ⊆ xs`; that
is to say, if one record can do everything that another record can, the former
is a subtype of the latter. As such, we should be able to provide an upcast
operator which "forgets" whatever makes one record different from another
(whether it be extra data, or different order).
...
So far, we've been working with the `PlainRec` type; but below that, there is
something a bit more advanced called `Rec`, which looks like this:

< data Rec :: [*] -> (* -> *) -> * where
<   RNil :: Rec '[] f
<   (:&) :: (r ~ (sy ::: t)) => f t -> Rec rs f -> Rec (r ': rs) f

The second parameter is a functor, in which every element of the record will be
placed. In `PlainRec`, the functor is just set to `Identity`. Let's try and
motivate this stuff with an example.

Let's imagine that we want to do validation on a record that represents a name
and an age:

> type Person = ["name" ::: String, "age" ::: Int]

We've decided that names must be alphabetic, and ages must be positive. For
validation, we'll use a type that's included here called `Result e a`, which
is similar to `Either`, except that its `Applicative` instance accumulates
monoidal errors on the left.
...
The results are as expected (`Success` for `goodPerson`, and a `Failure` with
one error for `badPerson`); but this was not very fun to build.

Further, it would be nice to have some notion of a **partial record**; that is,
if part of it can't be validated, it would still be nice to be able to access
the rest. **What if we could make a version of this record where the elements
themselves were validation functions, and then that record could be applied to a
plain one, to get a record of validated fields?** That's what we're going to do.

Vinyl provides a type of validators, which is basically a natural transformation
from the `Identity` functor to the `Result` functor, which we just used above.

< type Validator e = Identity ~> Result e

Let's parameterize a record by it: when we do, then an element of type `a`
...
And we can use the special application operator `<<*>>` (which is analogous to
`<*>`, but generalized a bit) to use this to validate a record:
...
give us a value of type `Result [String] (PlainRec Person)` using `run`:
...
The problem is then we can't do anything with the record that requires us to
know what its functor is. For instance, `cast` will fail. So, we might try to
provide a type annotation, but that can be a bit brittle and frustrating to have
to do. To alleviate this problem, `fixRecord` is provided:
