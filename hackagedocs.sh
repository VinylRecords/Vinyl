...
if [ "${S}" -eq "0" ]; then
    cd "dist/doc/html"
    DDIR="vinyl-${1}-docs"
    cp -r "vinyl" "${DDIR}" && tar -c -v -z --format ustar -f "${DDIR}.tar.gz" "${DDIR}"
    CS=$?
    if [ "${CS}" -eq "0" ]; then
        echo "Uploading to Hackage…"
        curl -X PUT -H 'Content-Type: application/x-tar' -H 'Content-Encoding: gzip' --data-binary "@${DDIR}.tar.gz" "http://${2}:${3}@hackage.haskell.org/package/vinyl-${1}/docs"
        exit $?
    else
